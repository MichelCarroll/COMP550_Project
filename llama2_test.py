...
LLAMA_2_13B_CHAT = "meta/llama-2-13b-chat:f4e2de70d66816a838a89eeeb621910adffb0dd0baba3976c96980970978018d"

ACTIVE_MODEL = LLAMA_2_70B_CHAT

negative = """
Warner Bros. reported a third-quarter loss of 17 cents a share on revenue of $9.98 billion.
"""
...
def classify(text): 
    output = replicate.run(
        ACTIVE_MODEL,
        input={
            "system_prompt": "You are a financial analyst, determining what effect a portion of an earnings call transcript will have on the company's stock price. Be as critical and skeptical as possible, not taking what they say at face value. Respond with one of: UP, DOWN, SAME", 
            "temperature": 0.25,
            "prompt": f"""
    ...
    SAME
    [INST]{text}[/INST]
    """, "stop_sequences": ".", "max_new_tokens": 10}
    )
...
def analyze(text): 
    output = replicate.run(
        ACTIVE_MODEL,
        input={
            "system_prompt": "You are a financial analyst, determining what effect a portion of an earnings call transcript will have on the company's stock price. Analyze the transcript, giving your impression of it.", 
            "temperature": 0.25,
            "prompt": f"""
    [INST]{text}[/INST]
    """, "stop_sequences": "", "max_new_tokens": 1000}
    )
